      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (shared, characterPairs) => {
            ...
            const chronicleDataList = characterPairs.map(([actorId, unique]) =>
              mapToCharacterData(shared, unique)
            );
